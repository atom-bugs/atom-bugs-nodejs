...
export class NodeDebuggerProtocol extends EventEmitter {
    ...
    connect(socketUrl) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.client.onerror = (error) => {
                if (this.retry < 3) {
                    setTimeout(() => {
                        resolve(this.connect(socketUrl));
                    }, 500);
                }
                else {
    ...
    getCallStack() {
        return this.callFrames.filter((frame) => {
            frame.location.script = this.getScriptById(parseInt(frame.location.scriptId));
            let sourceMap = frame.location.script.sourceMap;
            if (sourceMap) {
                let position = sourceMap.getOriginalPosition(frame.location.lineNumber, parseInt(frame.location.columnNumber));
                if (position) {
